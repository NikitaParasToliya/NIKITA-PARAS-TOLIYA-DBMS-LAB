...
SELECT M.MOVIE_TITLE FROM ACTOR A,MOVIE_CAST C,MOVIES M WHERE A.ACT_ID=C.ACT_ID AND C.MOVIE_ID=M.MOVIE_ID 
                                                        AND A.ACT_ID IN(SELECT ACT_ID FROM MOVIE_CAST GROUP BY ACT_ID HAVING COUNT(MOVIE_ID)>=2);
...
SELECT M.MOVIE_TITLE,MAX(R.RATING_STARS) AS MAXIMUM_RATING,COUNT(*) AS NUMBER_OF_RATINGS FROM MOVIES M,RATING R 
                                             WHERE M.MOVIE_ID=R.MOVIE_ID  GROUP BY R.MOVIE_ID HAVING COUNT(*)>0 ORDER BY M.MOVIE_TITLE;
